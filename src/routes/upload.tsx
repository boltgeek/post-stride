...
import { parseContent, addPosts, setPostsPerDay } from "@/lib/store";
    ...
    <div className="min-h-screen bg-background pb-24">
      <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        <div className="relative mb-4">
          ...
          />
          <button onClick={handlePaste} className="absolute top-3 right-3 text-muted-foreground hover:text-foreground transition-colors">
        ...
        {text.length > 0 && preview.length === 0 && (
          <Button onClick={handlePreview} variant="outline" className="w-full rounded-xl h-12 mb-4">
        ...
        {preview.length > 0 && (
          <div className="mb-6">
            <p className="text-sm font-semibold text-foreground mb-3">{preview.length} posts détectés</p>
        ...
        {text.length > 0 && (
          ...
          >
            <ArrowRight className="w-5 h-5 mr-2" /> {importing ? "Importation..." : "Importer et planifier"}
          </Button>
        )}
      </div>
      <BottomNav />
